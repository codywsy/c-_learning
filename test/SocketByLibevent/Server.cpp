...
bool Server::InitSocket() {
  ...
  int bind_result =
      bind(sock_fd_, (struct sockaddr *)&local_addr, sizeof(struct sockaddr));
  if (bind_result < 0) {
...
void Server::OnEvent(int sock_fd, short event) {
  ...
  int new_fd =
      accept(sock_fd_, (struct sockaddr *)&remote_addr, (socklen_t *)&sin_size);
  if (new_fd < 0) {
  ...
  std::cout << "build connection from client "
...
void Server::OnRead(bufferevent *bev) {
  ...
  // echo
  bufferevent_write(bev, data_.c_str(), data_.size());
}
...
void Server::OnError(bufferevent *bev, short what) {
  std::cout << "Get in Server::OnError, what=0x" << std::hex << what
            << std::endl;
  ...
  if (what & BEV_EVENT_EOF || what & BEV_EVENT_ERROR) {
    std::cout << "Some Error and Free bufferevent_free" << std::endl;

    // stop the loop of event_base_dispatch(base_)
